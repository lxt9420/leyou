...
public class SmsUtils {
    ...
    public  SendSmsResponse sendSms(String phoneNuber,String signName,String templateCode,String templateParam)  {
        String key=KEY_PREFIX + phoneNuber;
        //读取时间
        String lastTime=redisTemplate.opsForValue().get(key);
        if(StringUtils.isNotBlank(lastTime)){
            Long last = Long.valueOf(lastTime);
            if(System.currentTimeMillis() - last <SMS_MIN_INTERVAL_IN_MILLIS){
                 log.info("[短信服务]发送短信频率过高，被拦截，手机号码：{}",phoneNuber);
                 return null;
            }
        }
        try {
            ...
            //必填:待发送手机号
            request.setPhoneNumbers(phoneNuber);
            //必填:短信签名-可在短信控制台中找到
           ...
           // request.setOutId("123456");
            ...
            if(!"OK".equals(resp.getCode())){
                log.info("[短信服务] 发送短信失败，phoneNumber:{},原因:{}",phoneNuber,resp.getMessage());
            }
            //发送短信日志
            log.info("[短信服务],发送短信验证码，手机号：{}",phoneNuber);
            //发送短信成功后，写入redis,指定生存时间为1分钟
            redisTemplate.opsForValue().set(key,String.valueOf(System.currentTimeMillis()),1, TimeUnit.MINUTES);
            return resp;
        }catch (Exception e){
            log.error("[短信服务] 发送短信异常，手机号码:{}",phoneNuber,e);
            return null;
        }
    }
}
